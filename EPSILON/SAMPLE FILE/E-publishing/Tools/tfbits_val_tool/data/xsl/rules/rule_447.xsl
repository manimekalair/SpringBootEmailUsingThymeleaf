...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_447" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">447</xsl:variable>
		<xsl:variable name="rule_name">Check for disallowed children of &lt;back&gt; or &lt;book-back&gt;.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--
				-->
				<xsl:for-each select="//(back|book-back)/(title|bio)">
					<!--
					-->
					<hit>
						<xsl:copy-of select="."/>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:choose>
									<xsl:when test="./title">
										<xsl:text>Found &lt;title&gt; in book-back or back: "</xsl:text>
										<xsl:value-of select="./title"/>
									</xsl:when>
									<xsl:when test="./bio">
										<xsl:text>Found &lt;bio&gt; in book-back or back: "</xsl:text>
										<xsl:value-of select="substring(./bio,1,100)"/>
										<xsl:if test="string-length(./bio) gt 100">
											<xsl:text> ... </xsl:text>
										</xsl:if>
									</xsl:when>
								</xsl:choose>
