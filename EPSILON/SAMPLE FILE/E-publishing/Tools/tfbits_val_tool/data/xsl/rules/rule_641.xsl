...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_641" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">641</xsl:variable>
		<xsl:variable name="rule_name">Check for &lt;label&gt; elements.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//list[not(@list-type)]/list-item[not(label)]">
							<hit>
								<extract>
									<xsl:choose>
										<xsl:when test="string-length(.) le 60">
											<xsl:value-of select="."/>
										</xsl:when>
										<xsl:otherwise>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found list-item with no label in list without list-type attribute: "</xsl:text>
