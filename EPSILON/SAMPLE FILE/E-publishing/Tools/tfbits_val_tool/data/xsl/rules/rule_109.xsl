...
<!--
	...
	-->
	<xsl:template name="rule_109">
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">109</xsl:variable>
		<xsl:variable name="rule_name">Check CIP data.</xsl:variable>
		...
		<xsl:variable name="result">
			<xsl:for-each select="./book-meta/notes[@notes-type='imprint-page']/sec
				[matches(.,'library of congress cataloging in publication data', 'i')]">
				<tr>
					<xsl:copy-of select="$rule_details"/>
					<td class="fail">&lt;/sec sec-type="imprint-cip-data"&gt; contains text "Library of Congress Cataloging in Publication Data"</td>
				</tr>
			</xsl:for-each>
			<xsl:for-each select="./book-meta/notes[@notes-type='imprint-page']/sec
				[matches(.,'library of congress cataloging-in-publication data', 'i')]">
			...
			<xsl:if test="count(./book-meta/notes[@notes-type='imprint-page']/sec[@sec-type='imprint-cip-data']) &gt; 1">
				<tr>
					<xsl:copy-of select="$rule_details"/>
					<td class="fail">More than one &lt;/sec sec-type="imprint-cip-data"&gt; element</td>
				</tr>
			</xsl:if>
		</xsl:variable>
		<!-- DISPLAY RESULTS -->
