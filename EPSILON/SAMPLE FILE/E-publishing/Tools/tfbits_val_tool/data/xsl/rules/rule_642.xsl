...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_642" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">642</xsl:variable>
		<xsl:variable name="rule_name">Check for unnecessary &lt;label&gt; elements.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//list[@list-type]/list-item[label]">
							<hit>
								<extract>
									<xsl:choose>
										<xsl:when test="string-length(./*[not(self::label)][1]) le 60">
											<xsl:value-of select="./label"/>
											<xsl:text> </xsl:text>
											<xsl:value-of select="./*[not(self::label)][1]"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="./label"/>
											<xsl:text> </xsl:text>
											<xsl:value-of select="substring(./*[not(self::label)][1],1,60)"/>
											<xsl:text> ...</xsl:text>
										</xsl:otherwise>
									</xsl:choose>
								</extract>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found list-item with possible redundant label in list with list-type attribute: "</xsl:text>
