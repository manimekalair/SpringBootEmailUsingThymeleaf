...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_825" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">825</xsl:variable>
		<xsl:variable name="rule_name">Check for untagged &lt;publisher-loc&gt;.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!-- do not run test if there are no publisher-loc elements in references -->
				<xsl:choose>
					<xsl:when test="count(//ref//publisher-loc) = 0"/>
					<xsl:otherwise>
						<xsl:for-each select="//mixed-citation">
							<xsl:choose>
								<xsl:when test="(.//publisher-name) and not(.//publisher-loc)">
									<hit>
										<pub-name>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found &lt;publisher-name&gt; ("</xsl:text>
								<xsl:value-of select="./pub-name"/>
								<xsl:text>") but no &lt;publisher-loc&gt; in reference: "</xsl:text>
