...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_819" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">819</xsl:variable>
		<xsl:variable name="rule_name">Check &lt;year&gt; is a valid year.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//ref//year">
					...
					<!--  -->
					<xsl:choose>
						<xsl:when test="matches(.,'BCE?')"/>
						<xsl:when test="number(.) gt 2020">
							<hit>
								<year>
									<xsl:value-of select="."/>
								</year>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found invalid year: "</xsl:text>
