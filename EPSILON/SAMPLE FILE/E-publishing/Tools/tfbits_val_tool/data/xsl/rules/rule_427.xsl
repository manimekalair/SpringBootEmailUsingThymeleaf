...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_427" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">427</xsl:variable>
		<xsl:variable name="rule_name">Check for excess tagging.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--
				-->
				<xsl:for-each select="(//bold|//italic)">
					<!--
					-->
						...
						<xsl:when test="(name(following-sibling::node()[1]) = $name)">
							<hit>
								<text>
									...
									<xsl:copy-of select="following-sibling::node()[1]" copy-namespaces="no"/>
								</text>
								<location>
									<xsl:choose>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found excess </xsl:text>
								<xsl:value-of select="./text/*[1]/name()"/>
