...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xlink="http://www.w3.org/1999/xlink">
	...
	-->
	<xsl:template name="rule_607" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">607</xsl:variable>
		<xsl:variable name="rule_name">Validate artwork file names.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--
				-->
				<xsl:for-each select="(//graphic|//inline-graphic)/@xlink:href[matches(., '(\s|\..*\.|[^a-z0-9_\.-])')]">
					<!--
					-->
					
						<!--
						-->
						<!-- when xlink:href contains only a-z, 0-9, _, discard, else error -->
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found invalid artwork file name &#x2013; xlink:href="</xsl:text>
