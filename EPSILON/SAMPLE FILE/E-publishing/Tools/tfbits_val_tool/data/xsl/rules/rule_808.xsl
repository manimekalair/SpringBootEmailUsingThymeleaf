...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_808" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">808</xsl:variable>
		<xsl:variable name="rule_name">Check if reference has a name.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--  -->
				<xsl:for-each select="//mixed-citation[not(./person-group[./string-name or ./collab or ./anonymous])]">
					<hit>
						<text>
							<xsl:value-of select="."/>
						</text>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found reference with no &lt;person-group&gt; containing &lt;string-name&gt;, &lt;collab&gt; and/or no &lt;anonymous&gt; (</xsl:text>
