...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_640" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">640</xsl:variable>
		<xsl:variable name="rule_name">Check value of list-type.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//list/@list-type">
					<xsl:variable name="list-type" select="."/>
					...
					<xsl:analyze-string select="$list-type" regex="^(order|bullet|alpha-lower|alpha-upper|roman-lower|roman-upper|simple)$">
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found disallowed attribute value: list-type="</xsl:text>
								<xsl:value-of select="./list-type"/>
								<xsl:text>" (</xsl:text>
