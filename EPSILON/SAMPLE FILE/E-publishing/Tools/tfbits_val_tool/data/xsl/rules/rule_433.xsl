...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_433" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">433</xsl:variable>
		<xsl:variable name="rule_name">Check capitals in &lt;given-names&gt;.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//given-names[. = upper-case(.)]">
					<xsl:variable name="test-string" select="normalize-space(replace(., '[\s-]', ''))"/>
					<xsl:choose>
						<xsl:when test="string-length(replace($test-string, '[^\.]', '')) = string-length(replace($test-string, '[\.]', ''))"/>
						<xsl:otherwise>
							<hit>
								<text>
									<xsl:value-of select="."/>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>All-caps text (that does not match initials pattern) in &lt;given-names&gt; &#x2013; "</xsl:text>
