...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_622" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">622</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!-- select all nodes in table-wrap-foot that are not attrib -->
				<xsl:for-each select="//table-wrap-foot/node()[not(self::attrib)]">
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found possible table source not tagged &lt;attib&gt; in: </xsl:text>
