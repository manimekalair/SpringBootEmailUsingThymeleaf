...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	to 
	//book-meta/notes[@notes-type='imprint-page']/sec[@sec-type='imprint-bl-data']"
	-->

	<xsl:template name="rule_101">
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">101</xsl:variable>
		<xsl:variable name="rule_name">Check British Library data is present.</xsl:variable>
		...
		<xsl:variable name="result">
			<xsl:if
				test="not(//book-meta/notes[@notes-type = 'imprint-page']/sec[@sec-type = 'imprint-bl-data'])">
				<tr>
					<xsl:copy-of select="$rule_details"/>
					<td class="warning">No British Library data</td>
				</tr>
			</xsl:if>
		</xsl:variable>
		<!-- DISPLAY RESULTS -->
