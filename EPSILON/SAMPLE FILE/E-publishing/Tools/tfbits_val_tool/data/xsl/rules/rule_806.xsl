...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_806" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">806</xsl:variable>
		<xsl:variable name="rule_name">Validate value of publication-type.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//mixed-citation[@publication-type][not(@publication-type=('book', 'journal', 'web', 'conf-proc', 'newspaper', 'other'))]">
							<hit>
								<text>
									<xsl:value-of select="."/>
								</text>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found invalid value of @publication-type: "</xsl:text>
								<xsl:value-of select="./publication-type"/>
