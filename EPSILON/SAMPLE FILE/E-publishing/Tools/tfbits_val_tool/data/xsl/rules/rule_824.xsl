...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_824" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">824</xsl:variable>
		<xsl:variable name="rule_name">Check for untagged &lt;publisher-name&gt;.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:choose>
					<!--
					-->
					<!-- do not run test if there are no publisher-name elements in references -->
					<xsl:when test="count(//ref//publisher-name) = 0"/>
					<xsl:otherwise>
						<xsl:for-each select="//mixed-citation">
							<xsl:choose>
								<xsl:when test="(.//publisher-loc) and not(.//publisher-name)">
									<hit>
										<pub-loc>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found &lt;publisher-loc&gt; ("</xsl:text>
								<xsl:value-of select="./pub-loc"/>
								<xsl:text>") but no &lt;publisher-name&gt; in reference: "</xsl:text>
