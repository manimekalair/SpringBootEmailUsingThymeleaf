...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:xlink="http://www.w3.org/1999/xlink">
	...
	-->
	<xsl:template name="rule_601" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">601</xsl:variable>
		<xsl:variable name="rule_name">Check that all images in the XML have a corresponding file in the artwork folder.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--
				-->
				<xsl:for-each select="$artwork_xml//file">
					<xsl:choose>
						<xsl:when test=". = $filelist//folder[@path='artwork/']/filename"/>
						<xsl:otherwise>
							<hit>
								<xsl:value-of select="."/>
							</hit>
				...
				<xsl:otherwise>
					<xsl:element name="tr">
						...
						<xsl:element name="td">
							...
							<xsl:text>The following files are referenced in the xml but are missing from the artwork folder:</xsl:text>
								<xsl:element name="ul">
