...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_838" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">838</xsl:variable>
		<xsl:variable name="rule_name">Validate content of &lt;conf-name&gt;.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--
				-->
				<xsl:for-each select="//conf-name">
					<!--
					-->
					<xsl:choose>
						<xsl:when test="contains(lower-case(.),'presented at')">
							<hit>
								<xsl:copy-of select="."/>
								<location>
									<xsl:choose>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Invalid content of &lt;conf-name&gt; (contains "presented at") &#x2013; "</xsl:text>
