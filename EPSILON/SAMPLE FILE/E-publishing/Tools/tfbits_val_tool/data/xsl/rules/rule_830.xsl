...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_830" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">830</xsl:variable>
		<xsl:variable name="rule_name">Check mandatory elements for conference proceeding references.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//mixed-citation[@publication-type = 'conf-proc']">
					<xsl:choose>
						<xsl:when test="not(.//conf-name)">
							<hit>
								<text>
									<xsl:value-of select="."/>
								</text>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Missing &lt;conf-name&gt; in conf-proc reference: "</xsl:text>
