...
<!--
	...
	-->
	<xsl:template name="rule_454">
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">454</xsl:variable>
		<xsl:variable name="rule_name">Detect &lt;x&gt; elements.</xsl:variable>
		...
		<xsl:variable name="result">
			<xsl:for-each select="//x">
				<tr>
					...
					<td class="fail">
						<xsl:text>Found an &lt;x&gt; element: &lt;x&gt;</xsl:text>
						<xsl:value-of select="."/>
						<xsl:text>&lt;/x&gt;: </xsl:text>
						<xsl:choose>
