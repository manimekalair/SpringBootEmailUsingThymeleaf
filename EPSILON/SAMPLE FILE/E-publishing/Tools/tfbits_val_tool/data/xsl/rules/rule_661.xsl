...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_661" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">661</xsl:variable>
		<xsl:variable name="rule_name">Check for position of &lt;disp-quote&gt;.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--
				-->
				<!-- we want to ignore fn as often we get notes which are just disp-quotes -->
				<xsl:for-each select="//p[not(parent::fn)]/disp-quote">
					<!--
					-->
					<!-- pass preceding test to variable -->
					<xsl:variable name="preceding-text" select="preceding-sibling::text()[1]"/>
					<xsl:choose>
						<!--
						-->
						<!-- if there is preceding text, discard; else, this is a hit -->
						<xsl:when test="$preceding-text"/>
						<xsl:otherwise>
							<hit>
								<text>
									<xsl:choose>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found &lt;disp-quote&gt; nested in &lt;p&gt; element that is not preceded by any text: "</xsl:text>
