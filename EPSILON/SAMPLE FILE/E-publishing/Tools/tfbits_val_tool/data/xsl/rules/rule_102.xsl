...
<!--
	...
	<xsl:template name="rule_102">
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">102</xsl:variable>
		<xsl:variable name="rule_name">Check CIP data is present.</xsl:variable>
		...
		<xsl:variable name="result">
			<xsl:if test="not(//book-meta/notes[@notes-type='imprint-page']/sec[@sec-type='imprint-cip-data'])">
				<tr>
					<xsl:copy-of select="$rule_details"/>
					<td class="warning">No CIP data</td>
				</tr>
			</xsl:if>
		</xsl:variable>
		<!-- DISPLAY RESULTS -->
