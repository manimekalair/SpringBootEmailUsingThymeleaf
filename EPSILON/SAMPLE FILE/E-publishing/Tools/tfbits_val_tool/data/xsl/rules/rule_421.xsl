...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_421" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">421</xsl:variable>
		<xsl:variable name="rule_name">Check acknowledgements.</xsl:variable>
		...
		-->
		<!-- build list of text to match titles against -->
		...
		<xsl:variable name="hit_list">
			<xml>
				<!-- select all titles whose parent is not ack -->
				<xsl:for-each select="//title[not(parent::ack)]">
					<!-- set variables for testing and reporting -->
					<xsl:variable name="title_text" select="."/>
					<xsl:variable name="parent" select="name(parent::*)"/>
					...
					<!-- select each title that matches an entry in the match list -->
					<xsl:for-each select="$matches//text[. = lower-case($title_text)]">
						<hit>
							<title>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found acknowledgements (title: "</xsl:text>
								<xsl:value-of select="./title"/>
								<xsl:text>") with incorrect container element (</xsl:text>
								<xsl:value-of select="./parent"/>
								<xsl:text>) – </xsl:text>
