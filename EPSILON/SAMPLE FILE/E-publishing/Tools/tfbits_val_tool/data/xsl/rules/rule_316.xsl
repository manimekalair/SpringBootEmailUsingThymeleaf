...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_316" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">316</xsl:variable>
		<xsl:variable name="rule_name">Check book-part-type attribute value.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--
				-->
				<xsl:for-each select="//@book-part-type">
					<xsl:choose>
						<xsl:when test="matches(.,'^(chapter|entry|essay|lesson|letter|module|part|section|topic|unit|frontispiece|epigraph|bm-chapter)$')"/>
						<xsl:otherwise>
							<hit>
								<text>
									<xsl:value-of select="."/>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found invalid value of @book-part-type: "</xsl:text>
