...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_706" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">706</xsl:variable>
		<xsl:variable name="rule_name">Check if note citations are superscript.</xsl:variable>
			...
			<xml>
				...
				<xsl:for-each select="//xref[@ref-type='fn' and @rid]">
					...
					<xsl:choose>
						<!--
						-->
						<!-- discard when child is sup -->
						<xsl:when test="./sup"/>
						<!--
						-->
						<!-- discard when in an index -->
						<xsl:when test="ancestor::sec[@sec-type = 'index']"/>
						<!--
						-->
						<!-- discard when format is [1] -->
						<xsl:when test="matches(.,'^\[\d+\]$')"/>
						<!--
						-->
						<!-- if contains alphanumeric charatcers, it is a hit -->
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found note citation with no &lt;sup&gt; among its children &#x2013; rid="</xsl:text>
								<xsl:value-of select="./xref/@rid"/>
								<xsl:text>"; citation text: "</xsl:text>
