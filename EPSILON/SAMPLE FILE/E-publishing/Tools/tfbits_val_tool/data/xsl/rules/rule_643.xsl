...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_643" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">643</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				...
				<xsl:for-each select="//disp-quote[list][not(attrib)][following-sibling::*[1][.//italic] or following-sibling::*[1][matches(., '(^source|^after|^from)', 'i')]]">
					<hit>
						<text>
							<xsl:value-of select="."/>
						</text>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found possible list source not tagged &lt;attib&gt; in: </xsl:text>
