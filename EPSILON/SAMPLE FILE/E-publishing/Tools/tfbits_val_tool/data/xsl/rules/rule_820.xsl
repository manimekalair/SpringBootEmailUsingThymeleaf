...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_820" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">820</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//ref//edition">
					...
					<!--  -->
					<xsl:choose>
						<xsl:when test="matches(.,'[^A-Za-z]([Ee]|[Ee]d|[Ee]d\.|[Ee]dn|[Ee]dn\.|[Ee]dition)')">
							<hit>
								<edition>
									<xsl:value-of select="."/>
								</edition>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found "edition" text in &lt;edition&gt; ("</xsl:text>
