...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_623" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">623</xsl:variable>
		<xsl:variable name="rule_name">Check if floating tables should be anchored.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--
				-->
				<xsl:for-each select="//*:table-wrap[not(.//caption)][@position = 'float' or not(@position)]">
					<!--
					-->
					<hit>
						<location>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found table with no caption that is a floating table: </xsl:text>
