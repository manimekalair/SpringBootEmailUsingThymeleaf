...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_811" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">811</xsl:variable>
		<xsl:variable name="rule_name">Check for anonymous authors.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//mixed-citation[not(.//person-group/anonymous)]">
					...
					<xsl:analyze-string select="." regex="^([Aa]non\.?|[Aa]nonymous|[Uu]nknown)[\.,]*\s+">
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found untagged anonymous author: "</xsl:text>
