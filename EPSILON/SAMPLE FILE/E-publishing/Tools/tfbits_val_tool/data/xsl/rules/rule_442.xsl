...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_442" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">442</xsl:variable>
		<xsl:variable name="rule_name">Check for empty &lt;body&gt; element.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--
				-->
				<xsl:for-each select="//body[not(descendant::*)]">
					<!--
					-->
					<hit>
						<title>
							<xsl:value-of select="preceding-sibling::book-part-meta/title-group/title"/>
						</title>
						<location>
							<xsl:choose>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found empty &lt;body&gt; &#x2013; </xsl:text>
								<xsl:if test="not(./title = '')">
									<xsl:text>book-part title: "</xsl:text>
