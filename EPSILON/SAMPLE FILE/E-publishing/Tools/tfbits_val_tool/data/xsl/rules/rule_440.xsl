...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_440" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">440</xsl:variable>
		<xsl:variable name="rule_name">Check for &lt;break/&gt; in titles.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<!--
				-->
				<xsl:for-each select="//(title|book-title|subtitle)//break">
					<!--
					-->
							<hit>
								<text_before>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found break in &lt;</xsl:text>
								<xsl:value-of select="./name"/>
								<xsl:text>&gt; &#x2013; "</xsl:text>
