...
<!--
	...
	-->
	<xsl:template name="rule_842">
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">842</xsl:variable>
		<xsl:variable name="rule_name">Check for references in a &lt;ref-list&gt;.</xsl:variable>
		...
		<xsl:variable name="result">
			<xsl:for-each select="//ref-list[not(./ref or ./ref-list)]">
				<tr>
					<xsl:copy-of select="$rule_details"/>
					<td class="fail">
						<xsl:text>Found a &lt;ref-list&gt; that contains no &lt;ref&gt; elements: </xsl:text>
						<xsl:choose>
