...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:mml="http://www.w3.org/1998/Math/MathML">
	...
	-->
	<xsl:template name="rule_449" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">449</xsl:variable>
		<xsl:variable name="rule_name">Check for alternative graphic in maths.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//*[self::disp-formula or self::inline-formula][(./mml:math|./alternatives/mml:math)][not(./alternatives/(graphic|inline-graphic))]">
					<hit>
						<location>
							<xsl:choose>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found MathML in </xsl:text>
								<xsl:value-of select="concat('&lt;',./node,'&gt;')"/>
								<xsl:text> with no alternative graphical representation at: </xsl:text>
