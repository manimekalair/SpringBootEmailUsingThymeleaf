...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_417" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">417</xsl:variable>
		<xsl:variable name="rule_name">Check tagging of contributor biographies.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="/book/front-matter/bio">
					...
					<!-- test if bio contains no bold -->
					<xsl:if test="not(.//named-content[@content-type='bio-name'])">
						<hit>
							<type>bold</type>
							<xsl:copy-of select="$location"/>
						</hit>
							...
							<xsl:element name="td">
								...
								<xsl:choose>
									<xsl:when test="./type = 'title'">
										<xsl:text>bio element containing a title</xsl:text>
									</xsl:when>
									<xsl:when test="./type = 'bold'">
										<xsl:text>bio element which contains no named-content[@content-type='bio-name']</xsl:text>
									</xsl:when>
								</xsl:choose>
