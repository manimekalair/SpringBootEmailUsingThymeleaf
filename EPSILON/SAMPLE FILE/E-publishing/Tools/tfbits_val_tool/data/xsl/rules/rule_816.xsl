...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_816" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">816</xsl:variable>
		<xsl:variable name="rule_name">Check for untagged &lt;degrees&gt; element.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//mixed-citation/person-group/string-name/node()[not(self::degrees or self::given-names)]">
					...
					-->
					<!-- find prefix -->
					<xsl:analyze-string select="." regex="(^|\s+)(BA|B\.A|MA|M\.A|BSc|B\.Sc|MSc|M\.Sc|PhD|Ph\.D|DPhil|D\.Phil|MD|M\.D)($|\s+)">
					...
					<xsl:for-each select="$hit_list//hit">
						...
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found degree text ("</xsl:text>
