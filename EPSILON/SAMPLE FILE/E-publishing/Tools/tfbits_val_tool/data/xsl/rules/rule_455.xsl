...
<!--
	...
	-->
	<xsl:template name="rule_455">
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">455</xsl:variable>
		<xsl:variable name="rule_name">Detect &lt;self-uri&gt; in &lt;book-part-meta&gt;.</xsl:variable>
		...
		<xsl:variable name="result">
			<xsl:for-each select="//book-part-meta/self-uri">
				<tr>
					<xsl:copy-of select="$rule_details"/>
					<td class="fail">
						<xsl:text>Found a &lt;self-uri&gt; element: </xsl:text>
						<xsl:choose>
