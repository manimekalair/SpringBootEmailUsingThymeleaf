...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_438" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">438</xsl:variable>
		<xsl:variable name="rule_name">Check for all-caps where errors can be determined.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				...
				<xsl:for-each select="(//book-title|//title|//subtitle|//label|//source|//alt-title)">
					...
					-->
					<xsl:choose>
						<!-- test if string contains only capitals -->
						<xsl:when test=". = upper-case(.)">
							<!-- test if string contains at least 3 words -->
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found all-caps in element: "&lt;</xsl:text>
								<xsl:value-of select="./name"/>
								<xsl:text>&gt;</xsl:text>
