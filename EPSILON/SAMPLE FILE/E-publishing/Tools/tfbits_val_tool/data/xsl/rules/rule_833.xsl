...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions">
	...
	-->
	<xsl:template name="rule_833" match="*">
		<!--
		
		-->
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">833</xsl:variable>
		<xsl:variable name="rule_name">Check for &lt;chapter-title&gt; in journal references.</xsl:variable>
		...
		<xsl:variable name="hit_list">
			<xml>
				<xsl:for-each select="//mixed-citation[@publication-type = 'journal']">
					<xsl:choose>
					...
					<xsl:for-each select="$hit_list//hit">
						<xsl:element name="tr">
							...
							<xsl:element name="td">
								...
								<xsl:text>Found &lt;chapter-title&gt; ("</xsl:text>
								<xsl:value-of select="./chapter-title"/>
								<xsl:text>") in journal reference: "</xsl:text>
