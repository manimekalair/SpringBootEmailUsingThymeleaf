...
<!--
	...
	-->
	<xsl:template name="rule_456">
		<!-- SET RULE NUMBER, NAME, AND ERROR TYPE -->
		<xsl:variable name="rule_number">456</xsl:variable>
		<xsl:variable name="rule_name">Detect &lt;collection-meta&gt;.</xsl:variable>
		...
		<xsl:variable name="result">
			<xsl:for-each select="/(book|book-part-wrapper)/collection-meta">
				<tr>
					<xsl:copy-of select="$rule_details"/>
					<td class="fail">
						<xsl:text>Found a &lt;colection-meta&gt; element: </xsl:text>
						<xsl:choose>
