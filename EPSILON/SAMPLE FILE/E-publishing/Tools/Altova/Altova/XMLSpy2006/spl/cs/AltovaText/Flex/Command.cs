[call GenerateFileHeader("Command.cs")]
...
namespace Altova.TextParser.Flex
{
	...
	public class CommandBlock : Command
	{
		...
		public override bool WriteText(DocumentWriter doc)
		{
			...
			for (int i = 0; i < children.Length; ++i)
			{
				...
				DocumentWriter restDoc = new DocumentWriter(children\[i\], restStr);
				base.WriteText(doc);
	...
	public class CommandSplitSingle : Command
	{
		...
		public override bool WriteText(DocumentWriter doc)
		{
			...
			for (int i = 0; i < children.Length; ++i)
			{
				...
				if (first != null)
				{
					DocumentWriter firstDoc = new DocumentWriter(children\[i\], firstString);
					first.WriteText(firstDoc);
				}
				if (next != null)
				{
					DocumentWriter restDoc = new DocumentWriter(children\[i\], restString);
	...
	public class CommandSplitMultiple : CommandSplitSingle
	{
		...
		public override bool WriteText(DocumentWriter doc)
		{
			...
			for (int i = 0; i < children.Length; ++i)
			{
				...
				DocumentWriter part = new DocumentWriter(children\[i\], partString);
				if (next != null)
					next.WriteText(part);
		...
		private bool WriteTextMultilineVertical(DocumentWriter doc)
		{
			...
			for (int i = 0; i < children.Length; ++i)
			{
				...
				DocumentWriter colWriter = new DocumentWriter(children\[i\], col);
	...
	public class CommandCSV : Command
	{
		...
		/// constructor
		/// </summary>
		public CommandCSV(string name, ColumnDelimited\[\] columns, bool headers, string rowsep, string colsep, char quote, char escape) : base(name)
		{
		...
		public override bool ReadText(DocumentReader doc)
		{
			...
			while (range.IsValid())
			{
				...
				doc.GetOutputTree().EnterElement(name, NodeClass.Group);
				for (int col = 0; col < Math.Min(nColumns, values.Count); ++col)
				{
					if (col >= 0 && col < nColumns && columns\[col\].next != null)
					{
						DocumentReader cell = new DocumentReader(values\[col\].ToString(), doc.GetOutputTree());
		...
		private bool ReadNextCell(Range range, out string value)
		{
			...
			for (; p != range.end; ++p)
			{
				if (range.At(p) == rowDelimiter\[0\] && !insideQuotes && DelimiterMatches(range.GetContent(), p, range.end, rowDelimiter))
				{
					range.start = p + rowDelimiter.Length;
					return false; // last column
				}

				if (range.At(p) == colDelimiter\[0\] && !insideQuotes && DelimiterMatches(range.GetContent(), p, range.end, colDelimiter))
				{
	...
	public class CommandFLF : Command
	{
		...
		/// constructor
		/// </summary>
		public CommandFLF(string name, ColumnFixed\[\] columns, bool headers, Splitter splitter) : base(name)
		{
			this.columns = columns;
			this.splitter = splitter;
			this.columnHeaders = headers;
