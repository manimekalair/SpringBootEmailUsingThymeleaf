...
[if $DBLibraryCount > 0]
  public SchemaDuration(java.sql.Timestamp newvalue) {
    setValue( newvalue );
  }
[endif]
...
[if $DBLibraryCount > 0]
  public void setValue(java.sql.Timestamp newvalue) {
    if( newvalue == null ) 
      setEmpty();
    else {
      long l = newvalue.getTime() / 1000;
      setYear( (int)((l / 3600 / 24 / 31 / 12)) );
      setMonth( (int)((l / 3600 / 24 / 31) % 12));
      setDay( (int)((l / 3600 / 24) % 31));
      setHour( (int)((l / 3600) % 60));
      setMinute( (int)((l / 60 ) % 60));
      setSecond((int)(l % 60));
      setPartSecond( ((double)l) % 1.0);
      bNegative = false;
    }
  }
[endif]
...
[if $DBLibraryCount > 0]
  public void prepareStatement(java.sql.PreparedStatement stmt, int paramnr) throws java.sql.SQLException {
    java.sql.Timestamp dsql = new java.sql.Timestamp(getDate().getTime());
	if( isEmpty())
		stmt.setNull( paramnr, java.sql.Types.TIMESTAMP );
	else
	    stmt.setTimestamp(paramnr,dsql);
  }
[endif]
