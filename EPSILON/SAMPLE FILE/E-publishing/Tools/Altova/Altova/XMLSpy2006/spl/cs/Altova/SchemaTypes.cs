...
[if $DBLibraryCount > 0]
		// database support
		void AddCommandParameter(IDbCommand cmd);
[endif]
	}
...
[if $DBLibraryCount > 0]
		// ISchemaType
		public abstract void AddCommandParameter(IDbCommand cmd);
[endif]
		// ISchemaTypeCalendar
...
[if $DBLibraryCount > 0]
		public SchemaBinaryBase(IDataRecord recordset, string column)
		{
			int nColumn;
			try
			{
				nColumn = recordset.GetOrdinal( column );
			}
			...
			if( isNull = isEmpty = recordset.IsDBNull( nColumn ) )
				return;
			myValue = new byte\[(recordset.GetBytes( nColumn, 0, null, 0, int.MaxValue ))\];
			recordset.GetBytes( nColumn, 0, myValue, 0, myValue.Length );
			isNull = false;
		}
[endif]
...
[if $DBLibraryCount > 0]
		// ISchemaType
		public void AddCommandParameter(IDbCommand cmd)
		{
			if( cmd is OleDbCommand )
			{
				OleDbParameter param = new OleDbParameter(null, OleDbType.Binary, myValue.Length);
				if( !isEmpty && !isNull )
					param.Value = myValue;
				else
			...
			{
				OdbcParameter param = new OdbcParameter(null, OdbcType.Binary, myValue.Length);
				if( !isEmpty && !isNull )
					param.Value = myValue;
				else
					param.Value = DBNull.Value;
...
[if $DBLibraryCount > 0]
		public SchemaBoolean(IDataRecord recordset, string column)
		{
			int nColumn;
			try
			{
			...
			if( isEmpty = isNull = recordset.IsDBNull( nColumn ) )
				return;
			myValue = Convert.ToBoolean(recordset.GetValue(nColumn));
		}
[endif]
		#endregion //Constructors
...
[if $DBLibraryCount > 0]
		public void AddCommandParameter(IDbCommand cmd)
		{
			if( cmd is OleDbCommand )
			{
				OleDbParameter param = new OleDbParameter(null, OleDbType.Boolean);
				if( !isEmpty && !isNull )
					param.Value = myValue;
				else
					param.Value = DBNull.Value;
			...
			{
				OdbcParameter param = new OdbcParameter(null, OdbcType.Bit);
				if( !isEmpty && !isNull )
					param.Value = myValue;
				else
					param.Value = DBNull.Value;
...
[if $DBLibraryCount > 0]
		public SchemaInt(IDataRecord recordset, string column)
		{
			int nColumn;
			try
			{
			...
			if( isNull = isEmpty = recordset.IsDBNull( nColumn ) )
				return;
			myValue = Convert.ToInt32(recordset.GetValue(nColumn));
		}
[endif]
		#endregion //Constructors
...
[if $DBLibraryCount > 0]
		public void AddCommandParameter(IDbCommand cmd)
		{
			// Note: it's necessary to set precision and scale, otherwise some OLEDB-providers (f.e. MSADORA) would raise an error
			if( cmd is OleDbCommand )
			{
				OleDbParameter param = new OleDbParameter(null, OleDbType.Integer);
				param.Precision = 10;	
				param.Scale = 0;
				if( !isEmpty && !isNull  )
					param.Value = myValue;
			...
			{
				OdbcParameter param = new OdbcParameter(null, OdbcType.Int);
				param.Precision = 10;
				param.Scale = 0;
				if( !isEmpty && !isNull )
					param.Value = myValue;
...
[if $DBLibraryCount > 0]
		public SchemaLong(IDataRecord recordset, string column)
		{
			int nColumn;
			try
			{
			...
			if( isEmpty = isNull = recordset.IsDBNull( nColumn ) )
				return;
			myValue = Convert.ToInt64(recordset.GetValue(nColumn));
		}
[endif]
		#endregion //Constructors
...
[if $DBLibraryCount > 0]
		public void AddCommandParameter(IDbCommand cmd)
		{
			...
			if( cmd is OleDbCommand )
			{
				OleDbParameter param = new OleDbParameter(null, OleDbType.BigInt);
				param.Precision = 18;
				param.Scale = 0;
				if( !isEmpty && !isNull )
					param.Value = myValue;
			...
			{
				OdbcParameter param = new OdbcParameter(null, OdbcType.BigInt);
				param.Precision = 18;
				param.Scale = 0;
				if( !isEmpty && !isNull )
					param.Value = myValue;
...
[if $DBLibraryCount > 0]
		public SchemaDouble(IDataRecord recordset, string column)
		{
			int nColumn;
			try
			{
			...
			if( isNull = isEmpty = recordset.IsDBNull( nColumn ) )
				return;
			myValue = Convert.ToDouble(recordset.GetValue(nColumn));
		}
[endif]
		#endregion //Constructors
...
[if $DBLibraryCount > 0]
		public void AddCommandParameter(IDbCommand cmd)
		{
			...
			if( cmd is OleDbCommand )
			{
				OleDbParameter param = new OleDbParameter(null, OleDbType.Double);
				param.Precision = 18;
//				param.Scale = 18;
				if( !isEmpty && !isNull )
					param.Value = myValue;
			...
			{
				OdbcParameter param = new OdbcParameter(null, OdbcType.Double);
				param.Precision = 18;
//				param.Scale = 18;
				if( !isEmpty  && !isNull )
					param.Value = myValue;
...
[if $DBLibraryCount > 0]
		public SchemaDecimal(IDataRecord recordset, string column)
		{
			int nColumn;
			try
			{
			...
			if( isNull = isEmpty = recordset.IsDBNull( nColumn ) )
				return;
			myValue = Convert.ToDecimal(recordset.GetValue(nColumn));
		}
[endif]
		#endregion //Constructors
...
[if $DBLibraryCount > 0]
		public void AddCommandParameter(IDbCommand cmd)
		{
			...
			if( cmd is OleDbCommand )
			{
				OleDbParameter param = new OleDbParameter(null, OleDbType.Decimal);
				param.Precision = 18;
//				param.Scale = 18;
				if( !isEmpty && !isNull )
					param.Value = myValue;
			...
			{
				OdbcParameter param = new OdbcParameter(null, OdbcType.Decimal);
				param.Precision = 18;
//				param.Scale = 18;
				if( !isEmpty && !isEmpty )
					param.Value = myValue;
...
[if $DBLibraryCount > 0]
		public SchemaDateTime(IDataRecord recordset, string column)
			: base()
		{
			int nColumn;
			try
			...
			if( isNull = isEmpty = recordset.IsDBNull( nColumn ) )
				return;
			DateTime dt = Convert.ToDateTime(recordset.GetValue(nColumn));
			myValue = new DateTime(dt.Year, dt.Month, dt.Day, dt.Hour, dt.Minute, dt.Second, dt.Millisecond );
			eTZ = ETZ.Missing;
			offsetTZ = 0;
		}
[endif]
		...
		// ISchemaType
[if $DBLibraryCount > 0]
		public override void AddCommandParameter(IDbCommand cmd)
		{
			if( cmd is OleDbCommand )
			{
				OleDbParameter param = new OleDbParameter(null, OleDbType.Date);
				if( !isEmpty && !isNull )
					param.Value = myValue;
				else
					param.Value = DBNull.Value;
			...
			{
				OdbcParameter param = new OdbcParameter(null, OdbcType.DateTime);	// !!!!! or OdbcType.Date?
				if( !isEmpty && !isNull )
					param.Value = myValue;
				else
					param.Value = DBNull.Value;
...
[if $DBLibraryCount > 0]
		public SchemaDate(IDataRecord recordset, string column)
			: base()
		{
			int nColumn;
			try
...
[if $DBLibraryCount > 0]
		public override void AddCommandParameter(IDbCommand cmd)
		{
			if( cmd is OleDbCommand )
			{
				OleDbParameter param = new OleDbParameter(null, OleDbType.DBDate);
				if( !isEmpty && !isNull )
					param.Value = myValue;
				else
					param.Value = DBNull.Value;
			...
			{
				OdbcParameter param = new OdbcParameter(null, OdbcType.Date);
				if( !isEmpty && !isNull )
					param.Value = myValue;
				else
					param.Value = DBNull.Value;
...
[if $DBLibraryCount > 0]
		public SchemaTime(IDataRecord recordset, string column)
			: base()
		{
			int nColumn;
			try
			...
			if( isNull = isEmpty = recordset.IsDBNull( nColumn ) )
				return;
			DateTime dt = Convert.ToDateTime(recordset.GetValue(nColumn));
			myValue = new DateTime(myValue.Year, myValue.Month, myValue.Day, dt.Hour, dt.Minute, dt.Second, dt.Millisecond );
			eTZ = ETZ.Missing;
			offsetTZ = 0;
		}
[endif]
...
[if $DBLibraryCount > 0]
		public override void AddCommandParameter(IDbCommand cmd)
		{
			if( cmd is OleDbCommand )
			{
				OleDbParameter param = new OleDbParameter(null, OleDbType.DBTime);
				if( !isEmpty && !isNull )
					param.Value = new TimeSpan(0, myValue.Hour, myValue.Minute, myValue.Second, myValue.Millisecond);
				else
					param.Value = DBNull.Value;
				cmd.Parameters.Add( param );
			}
			...
			{
				OdbcParameter param = new OdbcParameter(null, OdbcType.Time);
				if( !isEmpty && !isNull )
					param.Value = new TimeSpan(0, myValue.Hour, myValue.Minute, myValue.Second, myValue.Millisecond);
				else
					param.Value = DBNull.Value;
				...
				cmd.Parameters.Add( new TimeSpan(0, myValue.Hour, myValue.Minute, myValue.Second, myValue.Millisecond) );
		}
[endif]
		// System.ICloneable
		public override object Clone()
...
[if $DBLibraryCount > 0]
		public SchemaDuration(IDataRecord recordset, string column)
			: base()
		{
			int nColumn;
			try
			...
			if( isNull = isEmpty = recordset.IsDBNull( nColumn ) )
				return;
			myValue = TimeSpan.FromTicks( recordset.GetInt64( nColumn ) );
		}
[endif]
	#endregion //Constructors
...
[if $DBLibraryCount > 0]
		public SchemaBase64Binary(IDataRecord recordset, string column)
			: base( recordset, column )
		{
		}
[endif]
		#endregion //Constructors;
...
[if $DBLibraryCount > 0]
		public SchemaHexBinary(IDataRecord recordset, string column)
			: base( recordset, column )
		{
		}
[endif]
...
[if $DBLibraryCount > 0]
		public SchemaString( IDataRecord recordset, string column)
		{
			int nColumn;
			try
			{
			...
			if( isNull = isEmpty = recordset.IsDBNull( nColumn ) )
				return;
			myValue = Convert.ToString(recordset.GetValue( nColumn ));
		}
[endif]
		#endregion Constructors
...
[if $DBLibraryCount > 0]
		public void AddCommandParameter(IDbCommand cmd)
		{
			// NOTE: Entering the length is essential, too. Otherwise ADO.Net will cut the string and use wrong values.
			if( cmd is OleDbCommand )
			{
				OleDbParameter param = new OleDbParameter(null, OleDbType.VarChar, myValue.Length > 0 ? myValue.Length : 1 );
				// OleDbType.VarChar is essential, it won't work with OleDbType.BSTR
				if( !isEmpty && !isNull )
					param.Value = myValue;
				else
					param.Value = DBNull.Value;
			...
			{
				OdbcParameter param = new OdbcParameter(null, OdbcType.VarChar, myValue.Length > 0 ? myValue.Length : 1 );
				if( !isEmpty && !isNull )
					param.Value = myValue;
				else
					param.Value = DBNull.Value;
