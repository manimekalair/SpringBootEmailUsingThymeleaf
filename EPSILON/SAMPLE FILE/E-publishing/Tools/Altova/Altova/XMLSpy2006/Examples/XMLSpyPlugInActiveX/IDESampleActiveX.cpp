...
HRESULT IDESampleActiveX::raw_OnEvent(long nEventID, SAFEARRAY** arrayParameters, IDispatch* pXMLSpy, VARIANT* pReturnValue)
{
	...
	if(m_ipSpyDispatch.p)
	{
		...
		CComPtr<XMLSpyLib::IDocument3>	ipDoc;
...
HRESULT IDESampleActiveX::raw_GetDescription(BSTR* pDescription)
{
	...
	CComBSTR	bstrDescr = _T("PlugIn with ActiveX;Sample PlugIn also having an ActiveX control");
...
void IDESampleActiveX::FillNamesMap(std::vector<TNamesMap>& vecNamesMap, XMLSpyLib::IXMLData* ipXMLData, std::vector<XMLSpyLib::SPYXMLDataKind>& vecKind)
{
	...
	if(ipNode.p)
	{
		...
		{
			CComPtr<XMLSpyLib::IXMLData>	ipChild;
